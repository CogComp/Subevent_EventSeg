...
class segment_loss_(torch.nn.Module):
    def __init__(self):
        super(segment_loss_, self).__init__()
        self.zero = Variable(torch.zeros(1), requires_grad=False)
        self.zero = self.zero.cuda()
        self.LogSigmoid = nn.LogSigmoid()
        
    def forward(self, alpha_logits):
        softmax_alpha = nn.LogSoftmax(1)(alpha_logits[:, 0:4])
        seg = self.LogSigmoid(alpha_logits[:, 4:])
        return torch.abs(softmax_alpha[:, 0] + softmax_alpha[:, 1] - seg)
